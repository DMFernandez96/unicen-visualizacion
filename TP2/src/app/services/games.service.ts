...
export class GamesService {
 ...
 getFiltered(name: string, category: string, ascendente: boolean, premium: boolean): Game[] {
  let gamesFiltered: Game[] = this.games.filter(
   (g) => g.name.toLowerCase().includes(name) && g.category == category && g.premium == premium
  );
