...
export class SearchComponent implements OnInit { 
	...
	premiumFilter: string="No especial"; 
	...
	ngOnInit(): void { 
		this.route.queryParamMap.subscribe((params) => { 
			...
			if (params.get("premium") == "Noespecial") 
			this.premium = "No especial"; 
				else this.premium = "Especial"; 
			} else this.premium = "No especial"; 
			this.fetchGames(); 
		}); 
	} 
	...
	getGames(category: string): Game[] { 
		if (this.category == "Todas" || this.category == category) 
			return this.gamesService.getFiltered( 
				...
				this.premium == "Especial" ? true : false, 
			); 
		else return []; 
	} 
	searchGame(): void { 
